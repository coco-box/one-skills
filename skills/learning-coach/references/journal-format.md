...
持续学习默认落盘。开始辅导前先选择 track；一旦产生计划、课件、测验、作答、批改、复盘或进度变化，就在当轮同步写入，不等待用户要求“记录本次”。

按产物类型归档：

| 学习活动 | 默认写入 |
|---|---|
| 新建或调整学习路线 | track 的 `plan.md`、`PROGRESS.md`、根 `INDEX.md` |
| 讲解课程或生成讲义 | 当前知识单元的 `lesson.md` |
| 出题、测验或布置作业 | `quiz.md` 或 `homework.md` |
| 用户提交作答 | `quiz-response.md`，或在适合合并时写入 `quiz-review.md` |
| 批改、答案解析、错因诊断 | `quiz-review.md` 或 `review.md`，并更新薄弱点 |
| 会话形成结论、决策或下一步 | 当前 session、`PROGRESS.md`、根 `INDEX.md` |

在同一轮会话中复用当前 session 和知识单元，按新信息增量更新，避免每个对话回合创建新文件。一次性孤立问答且无后续复用价值时不必建档。用户明确拒绝落盘时不创建或修改任何学习档案。
